...
COR_value<-COR_matrix[-c(1,2),-1]
COR_value<-t(apply(COR_value,1,as.numeric))
gene_name<-as.character(unlist(COR_matrix[-c(1,2),1]))
bacteria_name<-as.character(unlist(COR_matrix[1,-1]))
...
colours2 <- colorRampPalette(brewer.pal(7, "Set1"))(5)[as.numeric(as.factor(bacteria_name[which(Bacteria_cor>5)]))]
